...
Core tests take longer than any other Toklio tests, due to the high amount of computational work involved in validating core components.
...
To run only Toklio's core tests (after building):
...
To run only Toklio's crypto tests (after building):
...
```
tokliod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `toklio-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Toklio's hash tests (after building):
...
To run only Toklio's performance tests (after building):
...
To run only Toklio's unit tests (after building):
